...
sent_data()
{
  ...
  if [ -n "$IFDB_TOKEN" ]; then
    /usr/bin/curl -XPOST $CURL_ARGS --header "Authorization: Token ${IFDB_TOKEN}" "https://${IFDB_SERVER}/api/v2/write?org=${IFDB_ORG}&bucket=${IFDB_BUCKET}" --data-binary @/tmp/temp.txt
  else
